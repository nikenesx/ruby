...
class Employe
    ...
    def initialize name=nil, birtDate=nil, phoneNumber=nil, adress=nil, email=nil, pasport=nil, specialty=nil, workExperience=nil, nameLastWork=nil, position=nil, lastSalary=nil
        ...
        self.birthDate = birtDate if birtDate != nil
        self.phoneNumber = phoneNumber if phoneNumber != nil
        @adress = adress
        self.email = email if email != nil
        self.pasport = pasport if pasport != nil
        @specialty = specialty
    ...
    def correctName name
        if isName name
            ...
            name = name.downcase if name.downcase! != nil
            name = name.chars
    ...
    def isName name
        regIsName = Regexp.compile(/(^[а-яА-Яa-zA-Z]+\s*(-\s*[а-яА-Яa-zA-Z]+)?\s*[а-яА-Яa-zA-Z]+\s*(-\s*[а-яА-Яa-zA-Z]+)?\s*[а-яА-Яa-zA-Z]+\s*([а-яА-Яa-zA-Z]+)?$)/)
        name.strip!
...
employe1 = TestEmploye.new("Евгений Жидков Алексеевич", "14.05.1997", "89183456734",
...
class TerminalViewListEmployee
    def inputListEmployee
        ...
        while nextemp == true
            ...
            while emp.phoneNumber == nil
                print "Введите корректный номер телефона->"
                data = gets
                emp.phoneNumber = data
            end

            print "Введите ардес сотрудника->"
            data = gets.strip
            emp.adress = data
            ...
            print "Введите поспортные данные сотрудника->"
            data = gets.strip
            emp.pasport = data
            while emp.pasport == nil
                print "Введите корректную дату рождения->"
                data = gets
                emp.pasport = data
            end
            ...
            employersList.push(emp)
            puts "Ввести данные еще одного сотудника?\n1. Да\n2. Нет"
            f = gets.strip
            nextemp = false if f == "2" || f == "Нет"
        end
        employersList.each { |elem| elem.printEmployers}
    end
end
...
tv = TerminalViewListEmployee.new
tv.inputListEmployee